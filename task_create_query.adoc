...
summary: 'Créez des requêtes personnalisées pour rechercher, filtrer et analyser vos actifs et indicateurs d"infrastructure grâce à un contrôle précis, un regroupement flexible et un affichage des résultats personnalisable.' 
...
[role="lead"]
Créez des requêtes performantes pour explorer vos ressources d'infrastructure avec précision grâce à des options de filtrage, de regroupement et de tri flexibles. Élaborez des recherches ciblées pour identifier des ressources spécifiques, analyser les indicateurs de performance et organiser les résultats selon vos besoins.
