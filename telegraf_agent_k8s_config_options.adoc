...
[role="lead"]
Le link:task_config_telegraf_agent_k8s.html#configuringcustomizing-the-operator["Opérateur de surveillance Kubernetes"] Offre des options de personnalisation étendues via le fichier AgentConfiguration. Configurez les limites de ressources, les intervalles de collecte, les paramètres de proxy, les tolérances et les paramètres spécifiques aux composants pour Telegraf, Kube-State-Metrics, la collecte de journaux, le mappage de la charge de travail, la gestion des modifications, etc. afin d'optimiser la surveillance de votre environnement Kubernetes.
...
|  | non protégé | ee ce link:https://community.influxdata.com/t/updating-telegraf-to-version-1-29-5-crashes-kubernetes-pod/33376["Problème connu de Telegraf"]. Le paramètre _Unprotected_ indique à l'opérateur de surveillance Kubernetes d'exécuter Telegraf avec l' `--unprotected`indicateur. 
