...
TIP: Une fois que vous avez installé un agent sur un serveur ou une machine virtuelle, Data Infrastructure Insights collecte les métriques de ce système en plus de celles provenant des collecteurs de données que vous configurez. Ces métriques sont collectées en tant que métriques « Node ».
