...
* Le type ASSETS contient les API d'actif, de requête et de recherche. Les attributs et les métriques d'actif DII sont parfois appelés « odata ». Les API pour ceux-ci sont généralement accessibles via /rest/v1/asset ou /rest/v1/query. Voir le <<api-documentation-swagger,Documentation de l'API Swagger>> pour plus d'informations.
+
...
* Le type DATA INGESTION est utilisé pour récupérer et gérer les données d’ingestion et les métriques personnalisées, telles que celles provenant des agents Telegraf. Celles-ci sont parfois appelées « datalake » ou « lake » métriques. Les API pour celles-ci sont généralement accessibles via /rest/v1/lake. Voir le <<api-documentation-swagger,Documentation de l'API Swagger>> pour plus d’informations.
...
Notez que les types d'API auxquels un utilisateur a accès dépendent également du link:concept_user_roles.html["Rôle de l'utilisateur"] dont il dispose dans chaque ensemble de fonctionnalités Data Infrastructure Insights (Observabilité, Sécurité des charges de travail, Rapports).
