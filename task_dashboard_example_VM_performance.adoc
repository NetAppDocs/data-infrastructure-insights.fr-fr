...
summary: Tutoriel étape par étape pour créer un tableau de bord de performance de machine virtuelle avec des tableaux, des graphiques et des widgets qui comparent les indicateurs de la machine virtuelle et du stockage. 
...
[role="lead"]
Apprenez à créer un tableau de bord complet des performances des machines virtuelles grâce à ce tutoriel pas à pas. Vous créerez des widgets qui visualisent les données de performance des VM, comparent les métriques de stockage backend avec les performances frontend des VM et analysent les relations entre la latence des VM et la demande d'E/S pour de meilleures informations opérationnelles.
