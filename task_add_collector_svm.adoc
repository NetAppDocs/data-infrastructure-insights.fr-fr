...
. À tout moment, un collecteur de données doit être en cours d'exécution, un autre sera affiché comme _Arrêté_.
+
Le collecteur de données du SVM « en cours d'exécution » s'affichera comme _Running_.  Le collecteur de données du SVM « arrêté » actuel s'affichera comme _Arrêté_.

. À chaque basculement, l'état du collecteur de données passe de « en cours d'exécution » à « arrêté » et vice versa.
