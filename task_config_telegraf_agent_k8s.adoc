...
| Erreurs « ImagePullBackoff » observées | Ces erreurs peuvent survenir si vous utilisez un dépôt Docker personnalisé ou privé et que vous n'avez pas encore configuré l'opérateur de surveillance Kubernetes pour le reconnaître correctement. <<using-a-custom-or-private-docker-repository,En savoir plus>> à propos de la configuration pour un dépôt personnalisé/privé. 
