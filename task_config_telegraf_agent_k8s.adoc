...
NOTE: La mise à jour par bouton-poussoir n'est pas disponible pour les versions opérateur antérieures à 1.2057.0. Veuillez suivre les instructions de mise à jour manuelle ci-dessous pour passer à la dernière version. Après cela, les futures mises à jour pourront être effectuées à l'aide de la fonction de mise à jour par bouton-poussoir.
...
[[configuringcustomizing-the-operator]]
...
| Erreurs « ImagePullBackoff » observées | Ces erreurs peuvent survenir si vous utilisez un dépôt Docker personnalisé ou privé et que vous n'avez pas encore configuré l'opérateur de surveillance Kubernetes pour le reconnaître correctement. <<Utiliser un référentiel Docker personnalisé ou privé,En savoir plus>> à propos de la configuration pour un dépôt personnalisé/privé. 
...
| Je vois des messages d'erreur de Telegraf ressemblant à ce qui suit, mais Telegraf démarre et fonctionne : E! [inputs.kubernetes] Erreur dans le plugin : https://<IP>>:<port>>/pods[]statut HTTP 403 Forbidden | Ces messages peuvent s'afficher si vous utilisez une version de Kubernetes antérieure à 1.33. La ressource RBAC « nodes/pods » utilisée par telegraf n'existe pas sur ces versions. Consultez la section « For Kubernetes versions < 1.33 » du fichier operator-additional-permissions.yaml pour connaître les ressources RBAC requises sur les versions antérieures. 
|===
