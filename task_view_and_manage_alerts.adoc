...
Grâce aux fenêtres de maintenance, vous pouvez planifier des périodes de maintenance spécifiques durant lesquelles les notifications d'alerte sont désactivées pour les objets et les indicateurs de votre choix. Par exemple, vous pouvez désactiver les notifications d'alerte déclenchées par certains systèmes de stockage lorsque ces systèmes sont en période de mise à niveau planifiée.

Vous pouvez choisir de supprimer à la fois les alertes et les notifications pendant cette période, ou de supprimer uniquement les notifications tout en continuant à détecter les alertes.

image:Maintenance_Windows_example.png["Exemple de fenêtres de maintenance, width=400"]
...
Pour créer une fenêtre de maintenance :

. Accédez à *Observability > Alerts > Maintenance Windows* et cliquez sur *+ Maintenance Window*.
. Saisissez un *nom* pour la fenêtre de maintenance. Pour un résultat optimal, choisissez un nom clair et précis.
. Sélectionnez le *périphérique de stockage* ou l'*attribut d'objet*.
. Filtrer pour le ou les *moniteurs*. Les caractères génériques sont acceptés.
. Sélectionnez l’*Heure de début* et l’*Heure de fin* de la période de maintenance.
. Choisissez si vous souhaitez supprimer les *alertes et notifications*, ou *uniquement les notifications* (les alertes sont toujours détectées pendant la période concernée).
. Cliquez sur *Enregistrer* pour créer la fenêtre de maintenance.
