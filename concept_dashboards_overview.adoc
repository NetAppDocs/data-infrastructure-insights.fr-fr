...
summary: 'Créez des tableaux de bord personnalisés avec des widgets flexibles pour visualiser les données d"infrastructure, surveiller les tendances de performance et créer des vues opérationnelles adaptées à vos besoins spécifiques de surveillance.' 
...
[role="lead"]
Créez des tableaux de bord personnalisés qui vous fournissent les informations opérationnelles essentielles. Choisissez parmi plusieurs types de widgets—including tableaux, graphiques, jauges et diagrammes—pour visualiser vos données d'infrastructure exactement comme vous le souhaitez, ce qui permet une meilleure prise de décision et une surveillance plus efficace.
