...
NOTE: Lors de la création d'un moniteur, DII analyse les données de la semaine précédente pour détecter les pics ou les baisses significatifs ; ces données sont considérées comme des anomalies. Durant la première semaine suivant la création du moniteur (phase d'apprentissage), il est possible que les alertes présentent un risque de bruit accru. Pour atténuer ce bruit, seules les pics ou les baisses de plus de 30 minutes sont considérées comme des anomalies et génèrent des alertes. La semaine suivante, à mesure que davantage de données sont analysées, le bruit diminue généralement et seul un pic ou une baisse significatif ultérieur est considéré comme une anomalie.
