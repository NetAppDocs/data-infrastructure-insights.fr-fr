...
=== Visualisations d’assistant IA

Transformez instantanément des données complexes en informations graphiques claires. Demandez à l’Assistant IA de créer un tableau de bord et il visualisera la réponse à l’aide de graphiques et de tableaux interactifs, vous aidant à identifier les tendances et à résoudre les problèmes plus rapidement sans création manuelle de graphiques.

image:ai_assistant-column_graph.png["Visualisation de l'assistant IA"]



[[enhanced-vm-analyzer-filtering]]
=== Filtrage amélioré de l'analyseur de machine virtuelle

Une nouvelle option dans la vue VM Analyzer permet de masquer les volumes montés sans datastore, offrant ainsi une vue plus claire pour simplifier le dépannage des VMs.

image:vm_analyzer_datastore_only_view.png["curseur de vue réservé au datastore"]



[[ui-update-user-data-edit-mode]]
=== Mise à jour de l'interface utilisateur : Mode d'édition des données utilisateur

Préservez l'exactitude des informations relatives aux actifs tout en limitant les risques de modifications accidentelles. Les annotations et les attributs (par exemple, les « Données utilisateur ») s'affichent désormais en mode lecture seule par défaut, ce qui vous permet de consulter les détails avant de choisir le moment où vous souhaitez les modifier.

Modifiez uniquement lorsque nécessaire. Depuis un panneau coulissant, développez les détails de l'actif et sélectionnez l'icône en forme de crayon. La page d'accueil de l'actif s'ouvre en mode Édition, où vous pouvez mettre à jour les informations et sélectionner Enregistrer lorsque vous avez terminé.

Vous pouvez également ouvrir directement la page d'accueil de la ressource et sélectionner Modifier dans le coin supérieur droit.

image:UserDataEdit-Slideout.png["Modifier les données utilisateur dans le panneau latéral - Icône crayon, largeur=400"]

image:UserDataEdit-LP.png["Mode d'édition de la page d'accueil, largeur=200"]
...
=== Nouvelle page d'accueil de l'assistant IA

L'assistant IA de DII dispose désormais d'une page d'accueil dédiée, mettant l'analyse intelligente de l'infrastructure au premier plan. Démarrez instantanément de nouvelles conversations, reprenez votre dernière session en un clic ou commencez rapidement avec des suggestions d'invites.

image:AI_dashboard_page_default.png["Tableau de bord AI par défaut, largeur=800"]

Accédez à la page d'accueil depuis *AI Assistant* dans le menu de navigation ou ouvrez l'AI Assistant depuis n'importe où en utilisant l'icône « étoiles » en haut à gauche.

image:AI_dashboard_page_menu.png["Menu de navigation de l'assistant IA, largeur=600"] image:AI_assistant_access_button.png["Accès à l'assistant IA depuis n'importe où, width=300"]
...
=== Partagez des mises à jour importantes avec les messages de connexion

Tenez votre équipe informée et alignée grâce à des messages de connexion personnalisables. Communiquez des annonces importantes, des calendriers de maintenance ou des mises à jour de politiques directement aux utilisateurs lorsqu'ils se connectent. Avec la prise en charge du markdown, vous pouvez formater les messages pour plus de clarté et garantir que les informations critiques atteignent chaque utilisateur DII. Les comptes disposant de privilèges d'administrateur peuvent trouver la configuration dans le menu Admin sous Notifications.



[[more-workspace-less-clutter]]
=== Plus d'espace de travail, moins d'encombrement

Le menu de navigation DII a un tout nouveau look, correspondant désormais aux couleurs et au comportement de Console pour une expérience plus unifiée. Le menu principal, auparavant statique, sera réduit par défaut pour offrir un espace de travail plus ciblé et une utilisation plus efficace de l’espace à l’écran. Il suffit de cliquer sur l’icône hamburger (☰) dans le coin supérieur gauche pour y accéder.



[[expand-device-visibility-with-intelligent-ai-assistance]]
=== Élargissez la visibilité des appareils grâce à l'assistance intelligente de l'IA

Accélérez l'intégration de nouveaux appareils grâce à l'identification des appareils alimentée par l'IA. Data Infrastructure Insights analyse votre environnement et suggère des règles de résolution automatique des appareils, vous aidant à identifier et gérer plus d'appareils rapidement. Il vous suffit de vérifier et d'approuver les règles générées par l'IA et Data Infrastructure Insights s'occupe du reste, améliorant la visibilité et la gestion des appareils dans l'ensemble de votre infrastructure.



[[a-smarter-ai-assistant-for-global-teams]]
=== Un assistant IA plus intelligent pour les équipes internationales

L’assistant IA prend désormais en charge plus d’une douzaine de langues, permettant à votre équipe de communiquer naturellement dans sa langue préférée. Voir la liste complète link:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["ici"^].



[[new-monitors-for-infrastructure-attribute-changes]]
=== Nouveaux outils de surveillance des modifications des attributs d'infrastructure

Les moniteurs d'attributs sont désormais disponibles pour alerter automatiquement votre équipe en cas de déplacement de ressources entre data centers, de non-chiffrement d'un storage pool ou de modification d'attributs. Cela aide à maintenir les configurations de référence et à garantir qu’aucun détail ne passe inaperçu lors de nouveaux déploiements ou de changements.

image:attribute_monitor_in_list.png["Des moniteurs d'attributs sont ajoutés aux types de moniteur"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== Créez des alertes et des webhooks plus intelligents grâce à des messages dynamiques et riches en contexte

Lors de la création d'un moniteur ou d'un webhook, il vous suffit de saisir « % » dans la description de l'alerte ou dans l'URL, l'en-tête ou le corps du webhook pour sélectionner, parmi une liste dynamique, les attributs et annotations d'objet disponibles. Donnez à vos alertes le bon contexte pour améliorer la clarté, renforcer l'intégration des webhooks et accélérer les temps de réponse.

image:attribute_list_example.png["Liste des attributs dans la description d'un moniteur, largeur=400"]
...
=== DII connaît votre environnement mieux que jamais

Nous avons amélioré la façon dont l'IA de Data Infrastructure Insights comprend votre configuration. Elle s'appuie désormais sur un contexte plus riche concernant votre utilisation actuelle du service, la configuration et les tendances de performance avant de répondre. Le résultat est des recommandations initiales qui reflètent votre environnement plutôt qu’un point de départ générique, avec moins de suivis nécessaires pour obtenir une réponse utile.

Par exemple, accédez à un périphérique de stockage et l'Assistant IA vous fournira en un clic des réponses à des questions telles que :

* Quelle est l'évolution du taux d'utilisation de cette capacité de stockage, et quels StoragePools consomment le plus d'espace ?
* Pouvez-vous me montrer l'évolution du taux d'utilisation de cet appareil au cours des dernières 24 heures ?
* Quels volumes occupent le plus d'espace sur ce stockage ?
...
=== L'assistant IA est désormais généralement disponible

L'assistant IA Data Infrastructure Insights est désormais disponible pour tous les clients des sites DII hébergés sur AWS.

Notez que l'AI Assistant sur les sites Azure est encore en préversion. Surveillez cet espace pour l'annonce de la disponibilité générale.



[[infrastructure-insights-features-removed]]
=== Fonctionnalités d'Infrastructure Insights supprimées

Les fonctionnalités « Infrastructure Insights » de DII, _Shared Resources Under Stress_, _Kubernetes Namespaces Running out of Space_ et _Reclaiming ONTAP Cold Storage_, ne sont plus disponibles dans DII.
...
=== Réponses intelligentes instantanées concernant votre infrastructure

Découvrez le nouvel allié intelligent de votre infrastructure : le link:ai_for_dii_overview.html["*Data Infrastructure Insights Assistant IA d'analyse des infrastructures de données*"] transforme la façon dont vous interagissez avec vos données, transformant les requêtes complexes en conversations simples. Identifiez instantanément des schémas cachés, accélérez le dépannage et prenez des décisions fondées sur les données en toute confiance. Obtenez des réponses instantanées à des questions telles que :
...
L'aperçu de l'assistant IA de DII est désormais disponible pour la plupart des clients dont le service est hébergé sur les régions AWS ; la région Azure sera bientôt disponible. link:concept_preview_features.html["Apprendre encore plus"] à propos des fonctionnalités en avant-première de Data Infrastructure Insights.

image:ai_assistant_icon.png["icône d’assistant IA"]
...
==== Personnalisez les moniteurs système plus rapidement, sans duplication

Modifiez directement les moniteurs système par défaut selon vos besoins—changez les noms, les filtres et les regroupements en un seul endroit. Plus besoin de créer des moniteurs en double pour de simples ajustements. Une bannière pratique vous rappelle quels moniteurs ont été personnalisés, maintenant une visibilité complète sur votre configuration de surveillance.



==== Rationalisez la gestion des moniteurs grâce aux actions groupées

Gérez plusieurs moniteurs simultanément grâce aux actions groupées. Ajustez rapidement les paramètres de Health Monitoring pour des groupes de moniteurs, ce qui vous fait gagner du temps et réduit la complexité. Mettez à jour votre stratégie de surveillance en quelques secondes, pas en quelques minutes.



==== Concentrez vos scores de santé d'infrastructure sur ce qui compte le plus

Excluez des objets spécifiques de vos scores d'intégrité de l'infrastructure pour maintenir la précision et la pertinence. Lorsque vous consultez votre tableau de bord d'intégrité de l'infrastructure, supprimez instantanément les objets que vous souhaitez déprioriser sans modifier la configuration du moniteur. Cela concentre vos alertes sur les éléments d'infrastructure qui soutiennent vos priorités métier.

image:health_monitoring_exclude_object.png["exclure l'objet de la surveillance de l'état de santé"]
...
Les systèmes d'exploitation Linux suivants sont actuellement pris en charge avec Data Infrastructure Insights link:concept_acquisition_unit_requirements.html["Unités d'acquisition"]:

* AlmaLinux 9.5 et 10
* Centos (64 bits) : Stream 9
* Debian (64-bit) : 11 à 13
* OpenSUSE Leap 15.2 à 15.6, 16
* Oracle Enterprise Linux (64-bit) : 8.10, 9.2, 9.4, 9.6, 10
* Red Hat (64-bit) : 8.10, 9.2, 9.4, 9.6, 10
* Rocky 9.2 à 9.6
* SUSE Linux Enterprise Server 15 SP4 à 15 SP7, 16
* Ubuntu Server : 20.04, 22.04, 24.04 LTS
* SELinux sur les plateformes ci-dessus
...
[[try-cloud-insights-features-you-havent-used-yet]]
...
[[workload-security-pauseresume-a-collector]]
...
[[collecting-isilonpowerscale-logs-and-advanced-analytics-data]]
...
La fonctionnalité Reclaim ONTAP Cold Storage Insight prend désormais en charge FlexGroups et est maintenant disponible pour tous les clients.
...
| Les espaces de noms Kubernetes manquent d'espace | L'indicateur _Kubernetes Namespaces Running Out of Space_ vous offre une vue des charges de travail sur vos espaces de noms Kubernetes qui risquent de manquer d'espace, avec une estimation du nombre de jours restants avant que chaque espace ne soit plein. 


| Ressource partagée sous pression | L'analyse _Shared Resource Under Stress_ utilise l'IA/ML pour identifier automatiquement les zones de contention des ressources à l'origine de la dégradation des performances dans votre environnement, met en évidence les charges de travail impactées et fournit des recommandations pour y remédier, vous permettant ainsi de résoudre plus rapidement les problèmes de performance. 
...
[[hows-my-data-collection-health]]
...
[[whats-changed-in-my-environment]]
...
Cloud Insights présente régulièrement en avant-première de nouvelles fonctionnalités intéressantes. Si vous souhaitez découvrir une ou plusieurs de ces fonctionnalités, contactez votre équipe commerciale NetApp pour plus d'informations.
...
| Ressource partagée sous pression | L'analyse des ressources partagées sous tension utilise l'IA/ML pour identifier automatiquement les zones de contention des ressources qui entraînent une dégradation des performances dans votre environnement, met en évidence les charges de travail impactées et fournit des actions recommandées pour y remédier, vous permettant ainsi de résoudre plus rapidement les problèmes de performance. 
|===
...
'''nt1.png[Gestion des unités de widget de tableau]
