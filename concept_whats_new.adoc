...
Les systèmes d'exploitation suivants sont désormais pris en charge avec les unités d'acquisition Data Infrastructure Insights, en plus de ceuxlink:concept_acquisition_unit_requirements.html["déjà pris en charge"]-ci :
...
La fonctionnalité *VM Analyzer* de DII offre une vue complète et complète de vos environnements virtualisés. VM Analyzer simplifie le dépannage en cartographiant l'intégralité du chemin, des machines virtuelles au stockage, permettant ainsi aux utilisateurs d'identifier rapidement les problèmes de performances sur les disques virtuels, les machines virtuelles, les hôtes, les banques de données, les volumes et le stockage. Grâce à une vue topologique intuitive, au filtrage et à des indicateurs de performance détaillés, les administrateurs peuvent facilement identifier les goulots d'étranglement, analyser les déséquilibres de trafic et optimiser les performances globales du stockage.

image:vm_analyzer_example_with_panel.png["Carte topologique d'un système de stockage avec l'analyseur VM"]
...
=== Les nouvelles pages de destination vous permettent de rester dans la circulation

Les pages d'accueil repensées représentent tout ce dont vous avez besoin, à savoir des données récapitulatives, des graphiques de performances, des corrélations, des alertes et des changements, et des ressources clés, dans une seule vue.

Désormais, vous pouvez résoudre les problèmes plus rapidement et rester en mouvement, sans passer d'un onglet à l'autre ni perdre le contexte.

image:lp_new_design.png["nouveau design de page d'accueil avec placement du résumé et des sections sur les performances et des ressources supplémentaires"]



=== Amélioration de la sécurité des workloads

*Webhooks maintenant disponibles pour les alertes Workload Security*

Workload Security prend désormais en charge les notifications Webhook prêtes à l'emploi pour des outils tels que Slack, PagerDuty, Teams, etc. De plus, nous proposons des modèles personnalisables qui vous permettent de personnaliser le message ou de l'intégrer à toute autre application SIEM ou tierce. Acheminez les alertes critiques directement dans vos workflows de sécurité existants, afin que votre équipe puisse enquêter et répondre plus rapidement.

image:ws_webhook_slack_example.png["Exemple Webhook Slack pour Workload Security, width=400"]

*Migrer les collecteurs de sécurité de la charge de travail entre les agents*

Vous pouvez facilement migrer un collecteur Workload Security d'un agent à un autre, ce qui permet un équilibrage de charge efficace des collecteurs entre les agents. La migration est aussi simple que la modification du collecteur et la sélection de l'agent de destination dans la liste.

image:ws_migrate_collector_to_another_agent.png["migrez le collecteur, largeur=500"]
...
image:csv_export_async.png["icône bell icon vec liste des exportations .csv prêtes pour le téléchargement, width=400"]
