...
summary: 'Accédez directement à la base de données de reporting via des API, en contournant Cognos, pour des intégrations personnalisées, des rapports automatisés et des flux de travail d"analyse de données avancés.' 
...
[role="lead"]
Accédez directement à la base de données Data Infrastructure Insights Reporting via des API compatibles OData v4, sans passer par l’interface Cognos, pour une plus grande flexibilité. Cela permet des intégrations personnalisées, des flux de travail de reporting automatisés et une analyse de données avancée pour les utilisateurs ayant besoin d’un accès programmatique à leurs données d’infrastructure.
...
L’API Data Infrastructure Insights Reporting suit la norme link:https://www.odata.org/["OData v4"] (Open Data Protocol) pour l’interrogation de la base de données Reporting. Pour en savoir plus, consultez link:https://www.odata.org/getting-started/basic-tutorial/["ce tutoriel"] sur OData.
