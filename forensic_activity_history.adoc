...
* *Chemin* de l'entité - filtres de chemin d'accès de répertoire (chaîne de chemin se terminant par /) jusqu'à 4 répertoires profonds sont recommandés pour des résultats plus rapides. Par exemple, _"/home/userX/nested1/nested2/"_. Voir le tableau ci-dessous pour plus de détails.
...
| Path = «/home/userX/nested1/test » | Recherche récursive de tous les fichiers et dossiers sous le chemin regex donné(test* peut signifier fichier OU répertoire OU les deux) | Plus lent | La recherche de Regex répertoire+fichier sera plus lente que les recherches d'annuaire. 
