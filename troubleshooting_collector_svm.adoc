...
'''
*Problème :* Après l’ajout ou la mise à niveau d’un collecteur de données ONTAP SVM (Sécurité des charges de travail), le locataire affiche une alerte de surprovisionnement ou de dépassement de capacité d’abonnement. La capacité de stockage brute a augmenté alors qu’aucun nouveau matériel n’a été ajouté.

*Essayez ceci :* Le même espace de stockage peut être comptabilisé deux fois : une fois par un collecteur de cluster Observability ONTAP et une autre fois par un collecteur Workload Security SVM sur le même cluster.

Comparez la capacité de stockage brute indiquée par les collecteurs Observability ONTAP et les collecteurs Workload Security SVM. Pour chaque collecteur SVM, vérifiez si son cluster parent est déjà surveillé par un collecteur Observability ONTAP. Si c'est le cas, seule la capacité du cluster doit être prise en compte dans l'utilisation de l'abonnement.

Vérifiez si une migration SVM récente a eu lieu entre les clusters. Après la migration d'une SVM, les indicateurs de capacité peuvent rester associés à l'ancien cluster jusqu'à ce que Workload Security reçoive des informations mises à jour du collecteur.

Assurez-vous que le collecteur de données ONTAP SVM pour le SVM migré est en état d'exécution (RUNNING). Si nécessaire, redémarrez le collecteur afin que Workload Security puisse collecter les métriques correctes de cluster et de capacité après la migration.

Si la capacité du cluster et celle de la SVM apparaissent toutes deux dans le total, il est probable qu'elles soient comptabilisées deux fois. Cela se produit souvent peu après l'ajout, la mise à niveau, le redémarrage d'un collecteur SVM ou la migration d'une SVM entre clusters.

Si votre consommation dépasse toujours votre abonnement, ouvrez un ticket d’assistance NetApp en fournissant les données de consommation avant et après l’alerte, ainsi qu’une liste des collecteurs Observability et SVM avec leur capacité de stockage brute déclarée.

{vider}

'''
*Problème* : Un ou plusieurs collecteurs de données affichent Dégradé au lieu de En cours d’exécution, même si le service de collecte est opérationnel.

*À essayer* : SWS signale un état dégradé lorsqu’une ou plusieurs connexions FPolicy sur un nœud sont déconnectées. Il ne s’agit pas d’une panne — le collecteur continue de fonctionner — mais les événements d’audit de ces nœud(s) peuvent être perdus jusqu’au rétablissement de la connectivité.

ONTAP établit une connexion de canal de contrôle FPolicy à partir de chaque nœud disposant d'une interface logique de données opérationnelle pour la SVM. Un nœud sans interface logique de données locale ne peut pas se connecter. Voir :

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["Processus de communication entre le nœud et le serveur FPolicy ONTAP externe"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["Comment ONTAP FPolicy fonctionne avec des serveurs FPolicy externes"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["Exigences, considérations et bonnes pratiques pour la configuration de ONTAP FPolicy"^]


Dans l'interface utilisateur, notez quel(s) nœud(s) sont déconnectés, puis vérifiez si ces nœuds ont un LIF de données pour le SVM.

* Aucune LIF de données sur le(s) nœud(s) indiqué(s) : ignorez l’état dégradé. ONTAP peut afficher FPolicy comme déconnecté avec le motif « Aucune LIF locale présente pour se connecter au serveur FPolicy » lorsqu’aucune LIF de données locale n’est présente ; cela est attendu. Voir link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["KB : Serveur Fpolicy non connecté en raison de « no local lif present »"^]. Une future version de SWS arrêtera de marquer le collecteur comme dégradé dans ce cas.
* LIF de données présente sur le(s) nœud(s) listé(s) : examinez pourquoi FPolicy est déconnecté (LIF hors service / client data-fpolicy manquant, réseau/pare-feu ou redémarrage/basculement temporaire).
