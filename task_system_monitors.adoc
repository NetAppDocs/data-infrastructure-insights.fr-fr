...
summary: Examinez les moniteurs définis par le système pour les conditions de collecte des métriques, des journaux, des attributs et des données. 
---
= Moniteurs système
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Data Infrastructure Insights inclut des moniteurs système définis pour les métriques, les journaux, les attributs et les conditions de collecte de données. Les moniteurs système disponibles dépendent des collecteurs de données présents sur votre tenant. De ce fait, les moniteurs disponibles dans Data Infrastructure Insights peuvent changer à mesure que des collecteurs de données sont ajoutés ou que leurs configurations sont modifiées.
...
Les moniteurs définis par le système sont composés de métriques ou d'attributs et de conditions prédéfinis, ainsi que de descriptions et d'actions correctives par défaut, qui ne peuvent pas être modifiés. Vous _pouvez_ modifier la liste des destinataires des notifications pour les moniteurs définis par le système. Pour consulter les métriques ou attributs, les conditions, la description et les actions correctives, ou pour modifier la liste des destinataires, ouvrez un groupe de moniteurs définis par le système et cliquez sur le nom du moniteur dans la liste.
...
* *CVO Infrastructure* et *CVO Workload Examples* incluent des moniteurs correspondants pour Cloud Volumes ONTAP.
* L'état par défaut varie selon le moniteur. Consultez les tableaux ci-dessous et examinez un moniteur avant d'activer les notifications.
...
| Période de grâce de la licence NVMe-oF active | AVERTISSEMENT | Cet événement se produit quotidiennement lorsque le protocole NVMe over Fabrics (NVMe-oF) est utilisé et que la période de grâce de la licence est active.  La fonctionnalité NVMe-oF nécessite une licence après l'expiration de la période de grâce de la licence.  La fonctionnalité NVMe-oF est désactivée lorsque la période de grâce de la licence est terminée. | Contactez votre représentant commercial pour obtenir une licence NVMe-oF et l'ajouter au cluster, ou supprimez toutes les instances de configuration NVMe-oF du cluster. 


| Expiration de la période de grâce de la licence NVMe-oF | AVERTISSEMENT | La période de grâce de la licence NVMe over Fabrics (NVMe-oF) est terminée et la fonctionnalité NVMe-oF est désactivée. | Contactez votre représentant commercial pour obtenir une licence NVMe-oF et l'ajouter au cluster. 


| Début de la période de grâce de la licence NVMe-oF | AVERTISSEMENT | La configuration NVMe over Fabrics (NVMe-oF) a été détectée lors de la mise à niveau vers le logiciel ONTAP 9.5.  La fonctionnalité NVMe-oF nécessite une licence après l'expiration de la période de grâce de la licence. | Contactez votre représentant commercial pour obtenir une licence NVMe-oF et l'ajouter au cluster. 
...
| Échec de la récupération du volume persistant Kubernetes | Le volume n'a pas réussi sa récupération automatique. |  | Avertissement @ > 0 B 
...
| Boucle de redémarrage en échec (CrashLoopBackOff) du pod Kubernetes | Le Pod s'est écrasé et a tenté de redémarrer plusieurs fois. |  | Critique @ > 3 
...
| Volume découvert | Informatif | Ce message s'affiche lorsqu'un volume interne est découvert. 


| Volume modifié | Informatif | Ce message s'affiche lorsqu'un volume interne est modifié. 


| Nœud découvert | Informatif | Ce message s'affiche lorsqu'un nœud de stockage est découvert. 


| Nœud supprimé | Informatif | Ce message s'affiche lorsqu'un nœud de stockage est supprimé. 
...
[[attribute-monitors]]
=== Moniteurs d'attributs

Les moniteurs d'attributs évaluent les attributs de configuration signalés pour un objet. Contrairement aux moniteurs de métriques, ils ne comparent pas une métrique de série temporelle à un seuil numérique. Chaque condition ci-dessous génère une alerte _Warning_ lorsque l'attribut signalé correspond à un état non conforme ou indisponible.

Les anciens moniteurs de métriques de sécurité autonomes ONTAP Essentials ont été remplacés par les moniteurs d'attributs consolidés *ONTAP Cluster mal configuré* et *ONTAP Storage mal configuré*. Les moniteurs CVO correspondants appliquent les mêmes contrôles à Cloud Volumes ONTAP.



==== Moniteurs d'attributs ONTAP

|===
| Nom du moniteur | Groupe | État par défaut | Conditions | Action corrective 


| Cluster ONTAP mal configuré | Infrastructure ONTAP | Actif | Alertes déclenchées lorsqu'une ou plusieurs de ces 12 conditions de sécurité sont détectées : * AutoSupport HTTPS n'est pas configuré * Web HTTP est activé * SSH n'est pas sécurisé * La bannière de connexion est désactivée * La communication entre pairs du cluster n'est pas chiffrée * L'émetteur du certificat est auto-signé * L'administrateur local par défaut est activé * FIPS est désactivé * Le transfert des journaux n'est pas chiffré * Le hachage des mots de passe MD5 est activé * Remote Shell (RSH) est activé * Telnet est activé | Examinez les attributs concernés dans l'alerte. Désactivez les protocoles non sécurisés et le hachage des mots de passe, sécurisez SSH et la communication entre pairs, activez les contrôles de sécurité requis et remplacez les certificats auto-signés conformément à la politique de sécurité de votre organisation. 


| Stockage ONTAP mal configuré | Infrastructure ONTAP | Actif | Alertes lorsqu'une machine virtuelle de stockage ONTAP a la journalisation d'audit désactivée, que SSH n'est pas sécurisé ou que sa bannière de connexion est désactivée. | Activez la journalisation d'audit des Storage VM, sécurisez SSH et configurez une bannière de connexion appropriée. 


| Le cluster ONTAP manque de disques de rechange | Infrastructure ONTAP | Actif | Alertes lorsque le cluster signale que le nombre de disques de rechange est faible. | Examinez les disques de rechange par nœud et ajoutez de la capacité de rechange compatible pour maintenir la protection des données. 


| Service de stockage ONTAP indisponible | Exemples de charges de travail ONTAP | Actif | Alertes lorsqu'un service FCP ou iSCSI activé est indisponible sur une machine virtuelle de stockage ONTAP. | Vérifiez l'état de la machine virtuelle de stockage, du protocole et des LIF de données. Consultez les messages d'état et d'événements ONTAP récents, puis rétablissez le service SAN concerné. 


| Nœud ONTAP mal configuré | Exemples de charges de travail ONTAP | En pause | Alertes lorsque Cisco Discovery Protocol (CDP) est désactivé sur un nœud ONTAP. | Vérifiez les paramètres CDP du nœud et l'état du port réseau, puis activez CDP là où il est nécessaire pour la découverte du réseau et la surveillance de la topologie. 
|===


==== Moniteurs d'attributs Cloud Volumes ONTAP

|===
| Nom du moniteur | Groupe | État par défaut | Conditions | Action corrective 


| Cluster CVO mal configuré | Infrastructure CVO | Actif | Applique les contrôles de configuration de sécurité du cluster décrits pour *ONTAP Cluster Misconfigured* à Cloud Volumes ONTAP. | Examinez les attributs concernés et corrigez les paramètres de cluster signalés comme non sécurisés ou non conformes. 


| Stockage CVO mal configuré | Infrastructure CVO | Actif | Alertes lorsqu'une machine virtuelle de stockage Cloud Volumes ONTAP a la journalisation d'audit désactivée, que SSH n'est pas sécurisé ou que sa bannière de connexion est désactivée. | Activez la journalisation d'audit des Storage VM, sécurisez SSH et configurez une bannière de connexion appropriée. 


| Nœud CVO mal configuré | Exemples de charges de travail CVO | En pause | Alertes lorsque CDP est désactivé sur un nœud Cloud Volumes ONTAP. | Vérifiez les paramètres CDP et réseau du nœud, puis activez le CDP si nécessaire. 
|===


==== Moniteurs de configuration NTP

Les moniteurs NTP suivants restent disponibles en tant que moniteurs système autonomes pour ONTAP et Cloud Volumes ONTAP.

|===
| Nom du moniteur | Plate-forme | Seuil | Description du moniteur | Action corrective 


| Aucun serveur NTP n'est configuré | ONTAP | Avertissement @ < 1 | Le cluster ne dispose d'aucun serveur NTP configuré. Pour garantir la redondance et un service optimal, NetApp recommande d'associer au moins trois serveurs NTP au cluster. | Associez un serveur NTP au cluster en utilisant `cluster time-service ntp server create -server <ntp_server>`. 


| Le nombre de serveurs NTP est faible | ONTAP | Avertissement @ < 3 | Le cluster compte moins de trois serveurs NTP configurés. | Associez des serveurs NTP supplémentaires au cluster en utilisant `cluster time-service ntp server create -server <ntp_server>`. 


| Aucun serveur NTP n'est configuré sur CVO | Cloud Volumes ONTAP | Avertissement @ < 1 | Le cluster ne possède aucun serveur NTP configuré. | Associez un serveur NTP au cluster en utilisant `cluster time-service ntp server create -server <ntp_server>`. 


| Le nombre de serveurs NTP CVO est faible | Cloud Volumes ONTAP | Avertissement @ < 3 | Le cluster compte moins de trois serveurs NTP configurés. | Associez des serveurs NTP supplémentaires au cluster en utilisant `cluster time-service ntp server create -server <ntp_server>`. 
|===
...
| MetroCluster Lien d'initiateur FC déconnecté | CRITIQUE | L'adaptateur initiateur FC est en cause. | 1. Assurez-vous que le lien initiateur FC n’a pas été altéré. 2.  Vérifiez l'état opérationnel de l'adaptateur initiateur FC à l'aide de la commande « system node run -node local -command storage show adapter ». 


| MetroCluster Liaison d'interconnexion FC-VI déconnectée | CRITIQUE | Le lien physique sur le port FC-VI est hors ligne. | 1. Assurez-vous que la liaison FC-VI n’a pas été altéré. 2.  Vérifiez que l'état physique de l'adaptateur FC-VI est « Up » en utilisant la commande « metrocluster interconnect adapter show ». 3.  Si la configuration inclut des commutateurs fabric, assurez-vous qu'ils sont correctement câblés et configurés. 
